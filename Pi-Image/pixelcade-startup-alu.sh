...
while [  $STATE == "error" ]; do
    ...
    if [[ -f "$HOME/pixelcade/deletemeafterwificonnect.txt" ]]; then  #still exists so we are not wifi complete so show a different message
        curl  "http://localhost:8080/text?t=Using%20your%20phone%20or%20computer,%20connect%20to%20the%20%22Pixelcade%20Setup%22%20WiFi%20and%20then%20navigate%20to%20http%20:%20//%2010%20.%200%20.%200%20.%201%20%20from%20any%20web%20browser%20.%20%20Then%20select%20your%20normal%20WiFi%20network%20and%20password%20.&size=32&loop=10&color=cyan&font=Tall%20Films%20Fine&yoffset=-2"
    else
...
curl  "http://localhost:8080/text?t=Address:%20http://pixelcade.local:8080%20or%20http://$pi_ip:8080&c=green&size=24" #kill the Q
sh $HOME/pixelcade/system/update.sh #call auto-update
